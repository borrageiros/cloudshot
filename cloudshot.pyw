...
class Snipper(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None, flags=Qt.WindowFlags()):
        ...
        self.notifications = ALERT.lower() == 'true' if ALERT else False



    def winNotify(self, msg):
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() == Qt.Key_A and event.modifiers() == Qt.ControlModifier and os.getenv("TESSERACT") == "on":
            try:
                self.take_screenshot_and_convert_to_text()
                QtWidgets.QApplication.quit()
                print(notification)
                self.winNotify("Text copied to clipboard")
            except:
                self.winNotify("Error with Tesseract")
        ...
        # If Ctrl+S is pressed, save the screenshot to a file
            ...
            try:
                ...
                QtWidgets.QApplication.quit()
                self.winNotify("Image saved")
            except:
                self.winNotify("Error when trying to save the image")
        ...
        elif event.key() == Qt.Key_F and event.modifiers() == Qt.ControlModifier and os.getenv("SERVER") == "on":
            try:
                self.send_screenshot()
                QtWidgets.QApplication.quit()
                self.winNotify("Image uploaded")
            except:
                self.winNotify("Error when trying to upload the image")
        ...
        # If Ctrl+C is pressed, take the screenshot and put it into the clipboard
            ...
            try:
                ...
                QtWidgets.QApplication.quit()
                self.winNotify("Image copied to clipboard")
            except:
                self.winNotify("Error when trying to copy the image")
...
def listen_for_keypress():
    while True:
        if keyboard.is_pressed(os.getenv("ACTIVATE_KEY")):
            run_script_thread = threading.Thread(target=run_script)
            run_script_thread.start()
            run_script_thread.join()
